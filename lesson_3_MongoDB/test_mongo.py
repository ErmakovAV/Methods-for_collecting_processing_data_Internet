...
def parse():
    parser = argparse.ArgumentParser(description='Скрипт парсинга hh и поиска '
                                                 'информации по введённым значениям ')

    parser.add_argument("-p", "--position", nargs='+', type=str, default='',
                        help="Введите поисковые аргументы (default ' ')")
...
def validate_arguments(args):
    ...
    if args.position:
        ...
        input_string = input(
            'Введите поисковый аргумент вакансии: ')
...
def vacancy_search(db, args=None) -> list:
    ...
    # Создадим указатель на коллекцию в базе данных имя коллекции - выбранная вакансия
