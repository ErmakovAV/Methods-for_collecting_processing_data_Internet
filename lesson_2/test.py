...
def validate_arguments(args):
    ...
    if args.position:
        ...
        input_string = input('Выберите искомый элемент вакансии: ')
    ...
    if args.number:
        ...
        while True:
            try:
                pages = int(input('Введите количество тестируемых страниц: '))
            except ValueError:
...
def create_url(args, page=0) -> str:
    base_url = args['site']
    url = f'{base_url}search/vacancy?area=113&items_on_page=20&text={args["position"]}&page={page}'
    return url
